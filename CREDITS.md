# Credits

## Author

**Romain Peyrichou**
- GitHub: [@roomi-fields](https://github.com/roomi-fields)

## Acknowledgments

This project is built upon these excellent open-source projects:

### obsidian-content-os
- **Author**: [eharris128](https://github.com/eharris128)
- **Repository**: https://github.com/eharris128/obsidian-content-os
- **License**: MIT
- **Contribution**: Obsidian plugin structure, UI patterns, settings management

### substack-mcp-plus
- **Author**: [ty13r](https://github.com/ty13r)
- **Repository**: https://github.com/ty13r/substack-mcp-plus
- **License**: MIT
- **Contribution**: Substack API integration logic, content conversion patterns

### python-substack
- **Author**: [ma2za](https://github.com/ma2za)
- **Repository**: https://github.com/ma2za/python-substack
- **License**: MIT
- **Contribution**: Foundational Substack API research

## Third-Party Libraries

| Library | License | Purpose |
|---------|---------|---------|
| [Obsidian API](https://obsidian.md/) | Proprietary | Plugin framework |
| [TypeScript](https://www.typescriptlang.org/) | Apache-2.0 | Language |
| [esbuild](https://esbuild.github.io/) | MIT | Build tool |
| [Vitest](https://vitest.dev/) | MIT | Testing |
| [ESLint](https://eslint.org/) | MIT | Linting |

## Special Thanks

- The Obsidian team for creating an amazing note-taking app
- The open-source community for tools and inspiration
- Everyone who reports bugs and suggests features

---

Want to contribute? See [CONTRIBUTING.md](CONTRIBUTING.md)
